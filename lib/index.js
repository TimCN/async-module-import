 ...
 * @returns {string}
 */
function getModuleUlr(moduleId) {
    return window.webpackModuleFiles[moduleId];
}
export function asyncImport(rawModuleId) {
...
export default function asyncImportDefault(moduleId) {
    return asyncImport(moduleId).then(function (all) {
        return all["default"];
    });
}
